    ...
    const visitor = {
        ClassProperty(path) {
            ...
            if (
                compileTemplate &&
                key.name === 'template' &&
                outValue.type === 'TemplateLiteral'
            ) {
                const isStringTemplate =
                    Array.isArray(path.node.value.expressions) &&
                    path.node.value.expressions &&
                    path.node.value.expressions.length;

                const value =
                    Array.isArray(path.node.value.quasis) &&
                ...
                if (mode === 'strict' && isStringTemplate) {
                    throw new Error(
                        `${that.resourcePath}报错，template模板不支持字符串表达式`
                    );
                }
